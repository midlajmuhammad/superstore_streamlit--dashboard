...
with tab2:
    ...
    with col1:
        st.subheader("Top_10_Sub-Category")
        top_10_subcategory_by_sales_price=filtered.groupby('sub-category')['sales_price'].sum().nlargest(10).reset_index()
        cat_plot=top_10_subcategory_by_sales_price.plot(x='sub-category', y='sales_price', kind='barh')
...
with tab4:
    ...
    profit_margin = (
        filtered['profit'].sum() /
        filtered['sales_price'].sum()
    ) * 100
    if profit_margin>20:
        st.success(f"Healthy profit margin {profit_margin}")
    elif profit_margin >10:
        st.warning(f"moderate profit margin{profit_margin}")
    else:
        st.error(f"low profit margin{profit_margin}")
    
  
    # High Discount Orders
    st.info(
           f" {high_disc_n} orders above the "
           f"75th-percentile discount"
)
